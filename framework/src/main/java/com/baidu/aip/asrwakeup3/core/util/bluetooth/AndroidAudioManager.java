...
public class AndroidAudioManager {
    ...
    public void startBluetooth() {
        ...
        if (mBluetoothAdapter != null) {
            L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] Adapter found");
            if (mAudioManager.isBluetoothScoAvailableOffCall()) {
                L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] SCO available off call, continue");
            } else {
                L.w("AndroidAudioManager", "[Audio Manager] [Bluetooth] SCO not available off call !");
            }
            if (mBluetoothAdapter.isEnabled()) {
                L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] Adapter enabled");
    ...
    public synchronized void routeAudioToBluetooth() {
        if (!isBluetoothHeadsetConnected()) {
            L.w("AndroidAudioManager", "[Audio Manager] [Bluetooth] No headset connected");
            return;
        }
    ...
    private synchronized void changeBluetoothSco(final boolean enable) {
        ...
        if (enable && mIsBluetoothHeadsetScoConnected) {
            L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] SCO already enabled, skipping");
            return;
        } else if (!enable && !mIsBluetoothHeadsetScoConnected) {
            L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] SCO already disabled, skipping");
            return;
        }
            ...
            public void run() {
                ...
                do {
                    try {
                        ...
                        L.e("AndroidAudioManager", e.getMessage() + e);
                    }
    ...
    public void destroy() {
        if (mBluetoothAdapter != null && mBluetoothHeadset != null) {
            L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] Closing HEADSET profile proxy");
            mBluetoothAdapter.closeProfileProxy(BluetoothProfile.HEADSET, mBluetoothHeadset);
        }

        L.i("AndroidAudioManager", "[Audio Manager] [Bluetooth] Unegistering bluetooth receiver");
    ...
    public void enableHeadsetReceiver() {
        ...
        L.i("AndroidAudioManager", "[Audio Manager] Registering headset receiver");
    ...
    public void routeAudioToSpeakerHelper(boolean speakerOn) {
        L.w("AndroidAudioManager", "[Audio Manager] Routing audio to " + (speakerOn ? "speaker" : "earpiece"));
        if (mIsBluetoothHeadsetScoConnected) {
            L.w("AndroidAudioManager", "[Audio Manager] [Bluetooth] Disabling bluetooth audio route");
